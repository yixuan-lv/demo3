...
相同基座、相同 LoRA 配置（rank16 / all target）、相同 batch（4）、相同 lr（5e-5）、相同 epoch（3），只改变**是否 4-bit 量化**：

| 方案 | 量化 | Precision | Recall | F1 | 训练显存 | 训练时长 |
|------|------|-----------|--------|-----|----------|----------|
| LoRA | BF16 | 0.8433 | 0.8424 | 0.8429 | 19.48 GB | 67.4 min |
| **QLoRA** | 4-bit NF4 | 0.8438 | 0.8450 | **0.8444** | **14.51 GB** | 61.4 min |

**结论**：QLoRA 相比全精度 LoRA 节省 **4.97 GB 显存（-25.5%）**，F1 反而高 0.15，训练时间更短。**QLoRA 在显存、速度、性能三方面均优于全精度 LoRA。**
...
| 配置 | 参考指标 | 本实验 |
|------|---------|--------|
| Qwen2.5-7B-LoRA (batch 4) | 显存 27G，F1 84% | 显存 **19.48G**，F1 **84.29%** |
...
| 实验 | rank | lr | 量化 | Precision | Recall | F1 | 训练显存 | 时长 |
|------|------|-----|------|-----------|--------|-----|----------|------|
| baseline | 16 | 5e-5 | 4-bit | 0.8438 | 0.8450 | **0.8444** | 14.51G | 61.4min |
| rank8 | 8 | 5e-5 | 4-bit | 0.8441 | 0.8431 | **0.8436** | 14.28G | 61.2min |
| rank32 | 32 | 5e-5 | 4-bit | 0.8474 | 0.8434 | **0.8454** | 15.28G | 61.3min |
| lr1e5 | 16 | 1e-5 | 4-bit | 0.8026 | 0.8018 | **0.8022** | 14.51G | 61.2min |
| lr1e4 | 16 | 1e-4 | 4-bit | 0.8475 | 0.8389 | **0.8432** | 14.51G | 60.8min |
| lora | 16 | 5e-5 | BF16 | 0.8433 | 0.8424 | **0.8429** | 19.48G | 67.4min |
...
1. **QLoRA 优于全精度 LoRA**：省 4.97 GB 显存（-25.5%），F1 反而高 0.15，训练时长更短。

2. **学习率是性能最敏感的变量**：lr=1e-5 相比 5e-5 掉 4.22 个点，**最优 lr ≈ 5e-5 ~ 1e-4**。

3. **LoRA rank 收益边际递减**：rank 8→16→32，F1 从 0.8436 → 0.8444 → 0.8454，整体差异 < 0.2 个点，**rank=16 已基本够用**。

4. **显存对 rank 非线性**：rank 8 ≈ rank 16（均 ~14.5G），rank 32 涨到 15.28G。

5. **学习率不影响显存**：lr 1e-5 / 5e-5 / 1e-4 三个实验显存一致（14.51G）。
